...
class RetentionManager:
    ...
    def run_retention(self, remove_duplicates: bool = True, remove_low_scoring: bool = True, 
                     low_scoring_percentile: float = 25.0) -> Dict[str, Any]:
        ...
        try:
            ...
            if remove_duplicates:
                ...
                removal_reasons.update({trace_id: "duplicate" for trace_id in duplicate_ids})
            ...
            disk_savings_mb = initial_disk_usage["total_mb"] - final_disk_usage["total_mb"]
            ...
            output_data = {
                ...
                "duplicates_removed": len([tid for tid, reason in removal_reasons.items() if reason == "duplicate"]),
                "low_scoring_removed": len([tid for tid, reason in removal_reasons.items() if reason == "low_score"]),
...
def main():
    ...
    parser.add_argument("--report", action="store_true", help="Generate retention report")
    ...
    parser.add_argument("--no-low-scoring", action="store_true", help="Skip low-scoring removal")
    parser.add_argument("--percentile", type=float, default=25.0, help="Percentile for low-scoring removal")
