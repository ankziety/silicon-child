...
class AdaptiveResearchLoop:
    ...
    def __init__(self, store: Any, headless: bool = False):
        ...
        self.browser = VisionBrowser(store, vision_config=vision_config, headless=headless)
    ...
    def research_question(self, question: str) -> ResearchSession:
        ...
        self.reasoning_engine.log_thought(
            "observation",
            f"Starting research on: {question}",
            confidence=1.0
        )
        
        # Generate initial search queries
        initial_queries = self._generate_initial_queries(question)
        ...
        while self.iteration_count < self.max_iterations:
            ...
            search_targets = self.reasoning_engine.get_next_search_targets(max_targets=3)
            ...
            self.current_session.reasoning_summary = self.reasoning_engine.get_reasoning_summary()
            self.current_session.learning_stats = self.continuous_learner.get_learning_stats()
...
<queries>
  ...
  <query>specific search query 5</query>
        ...
        try:
            # Use aggregator if available via browser (vision browser holds aggregator)
            if hasattr(self, 'browser') and getattr(self.browser, 'aggregator', None):
                response = self.browser.aggregator.generate(query_prompt)
            else:
    ...
    def _research_knowledge_gap(self, gap):
        ...
        session_id = self.browser.start_vision_session(
            user_goal=f"Research: {gap.question}",
        ...
        try:
            ...
            print(f"Vision session completed with {len(session.actions_performed)} actions")
    ...
    def _extract_page_content(self, page_analysis, gap):
        ...
        try:
            ...
            if current_url and current_url not in self.current_session.sources_used:
                ...
                if page_content:
                    # Parse the content
                    parsed_content = self.parser.parse(current_url, page_content, "text/html")
    ...
    def _fetch_and_analyze_url(self, url: str, gap):
        ...
        try:
            ...
            # Parse content
            parsed_content = self.parser.parse(result.url, result.content, result.mime_type)
    ...
    def _form_conclusions(self):
        ...
        for hypothesis in hypotheses:
            ...
            if evidence:
                # Form conclusion
                conclusion_id = self.reasoning_engine.form_conclusion(hypothesis, evidence)
                
                # Add to session conclusions
                conclusion_thought = next(
                    (t for t in self.reasoning_engine.thoughts if t.id == conclusion_id),
                    None
                )
    ...
    def _generate_final_answer(self) -> str:
        ...
        context = {
            ...
            "conclusions": self.current_session.conclusions,
            "sources": self.current_session.sources_used,
