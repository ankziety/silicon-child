...
class TestLLMJudges:
    ...
    def test_judge_missing_api_key(self):
        ...
        with pytest.raises(
            EvaluationError, match="COHERE_API_KEY environment variable not set"
        ):
...
class TestJobLogging:
    ...
    def test_evaluation_job_logging(self, temp_store):
        ...
        jobs = temp_store.conn.execute(
            "SELECT * FROM jobs WHERE type = 'eval'"
        ).fetchall()
        assert len(jobs) == 1
    ...
    def test_promotion_job_logging(self, temp_store):
        ...
        eval_jobs = temp_store.conn.execute(
            "SELECT * FROM jobs WHERE type = 'eval'"
        ).fetchall()
        promote_jobs = temp_store.conn.execute(
            "SELECT * FROM jobs WHERE type = 'promote'"
        ).fetchall()
